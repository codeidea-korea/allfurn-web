<div class="message__section">
    ...
    <div class="content">
        ...
        @if($company->company_type === 'A')
            <div class="content__head">
                <div class="content__info">
                    <figure class="logo"><img src="{{ $company->profile_image }}"></figure>
                    <h2 class="content__title">올펀</h2>
                    @if($company->is_alarm === 'Y')
                        <div class="alarm alarm--on">알림 켜짐</div>
                    @else
                        <div class="alarm alarm--off">알림 꺼짐</div>
                    @endif
                </div>
                <div class="content__head-button">
                    <a role="button" class="head-button dialogue-insearch-toggle" aria-hidden="true" tabindex="-1"></a>
                    <a role="button" class="head-button usermenu-toggle" data-event-target="usermenu" aria-checked=""><div class="ico__more"><span class="a11y">더 보기</span></div></a>
                </div>
                <div class="content__usermenu" aria-hidden="true">
                    <a role="button" class="usermenu__item" id="alarmBtn" onclick="toggleAlarmModal('{{ $company->company_type }}','{{ $company->idx }}')">알림{{ $company->is_alarm === 'Y' ? '끄기' : '켜기' }}</a>
                </div>
                <div class="dialogue-insearch" aria-hidden="true" tabindex="-1"></div>
            </div>
            
            
        @elseif($company->company_type === 'W')
            <div class="content__head">
                <div class="content__info">
                    <figure class="logo"><img src="{{ $company->profile_image }}"></figure>
                    <h2 class="content__title">{{ $company->company_name }}</h2>
                    @if($company->is_alarm === 'Y')
                        <div class="alarm alarm--on">알림 켜짐</div>
                    @else
                        <div class="alarm alarm--off">알림 꺼짐</div>
                    @endif
                    <a role="button" class="head-button navi-toggle" aria-checked=""><div class="ico__arrow--down14"><span class="a11y">더 보기</span></div></a>
                </div>
                <div class="content__head-button">
                    <a role="button" class="head-button dialogue-insearch-toggle" aria-checked=""><div class="ico__search ico__search--gray"><span class="a11y">검색</span></div></a>
                    <a role="button" class="head-button usermenu-toggle" data-event-target="usermenu" aria-checked=""><div class="ico__more"><span class="a11y">더 보기</span></div></a>
                </div>
                <div class="content__navi" aria-hidden="true">
                    <div class="row">{{ $company->business_address }} {{ $company->business_address_detail }}</div>
                    <a href="tel:{{ $company->phone_number }}" class="row tel">{{ $company->phone_number }}</a>
                    <a href="/wholesaler/detail/{{ $company->idx }}" class="row navi-link">업체 자세히 보기<span class="ico__arrow--right14"></span></a>
                </div>
                <div class="content__usermenu" aria-hidden="true">
                    <a role="button" class="usermenu__item" id="alarmBtn" onclick="toggleAlarmModal('{{ $company->company_type }}','{{ $company->idx }}')">알림{{ $company->is_alarm === 'Y' ? '끄기' : '켜기' }}</a>
                    <a role="button" class="usermenu__item" onclick="reportModal('{{ $company->idx }}','{{ $company->company_type }}')">신고하기</a>
                </div>
                <div class="dialogue-insearch" aria-hidden="true">
                    <a role="button" class="head-button dialogue-insearch-close"><div class="ico__arrow--big_left24"><span class="a11y">뒤로 가기</span></div></a>
                    <div class="textfield">
                        <div class="textfield__icon ico__search"><span class="a11y">검색</span></div>
                        <input type="text" class="textfield__search" data-room-idx="{{ $room_idx }}" name="chatting_keyword" id="chatting_keyword" placeholder="대화 내용을 검색해주세요." />
        ...
        @else
            <div class="content__head">
                ...
                <div class="content__navi" aria-hidden="true">
                    <div class="row">{{ $company->locations }}</div>
                    <div class="row cate">
                        @foreach(explode(',',$company->category_names) as $category)
                        <div class="cate__badge">{{ $category }}</div>
                        @endforeach
                    </div>
                    <!-- <a href="/company/{{ $company->idx }}" class="row navi-link">업체 자세히 보기<span class="ico__arrow--right14"></span></a> -->
                </div>
                <div class="content__usermenu" aria-hidden="true">
                    <a role="button" class="usermenu__item" id="alarmBtn" onclick="toggleAlarmModal('{{ $company->company_type }}','{{ $company->idx }}')">알림{{ $company->is_alarm === 'Y' ? '끄기' : '켜기' }}</a>
                    <a role="button" class="usermenu__item" onclick="reportModal('{{ $company->idx }}','{{ $company->company_type }}')">신고하기</a>
        ...
        <div class="section">
            ...
            <div id="dialogue" class="dialogue">
                ...
                @foreach($chatting as $chat)
                    ...
                    @if($loop->first || $chatting[$loop->index-1]->message_register_day != $chat->message_register_day)
                        <div class="chat-box date"><span>{{ $chat->message_register_day }} {{ $day[date('w', strtotime($chat->register_time))] }}요일</span></div>
                    @endif
                    
                    
                    @if($chat->type == '1')
                    
                        <div class="chat-box chat-box--{{ $chat->arrow }} {{ $chat->arrow === 'left' ? 'chat-box--avatar' : '' }}" tabindex="0">
                            
                            <div class="chat chat--{{ $chat->arrow }}">
                                
                                @if($keyword)
                                
                                    {!! str_replace($keyword, '<div class="selection">'.$keyword.'</div>', $chat->content) !!}
                                    
                                @else
                                
                                    @if(preg_match('/\b(?:https?:\/\/)?(?:www\.)?\S+\.(?:com|org|net)\b/', $chat->content, $matches))
                                        <a href="{{ (strpos($matches[0], 'http') === 0) ? $matches[0] : 'http://' . $matches[0] }}" target="_blank" style="color:blue">{{ $chat->content }}</a>
                                    @else
                                        {{ $chat->content }}
                                    @endif
                            ...
                            @if($chat->arrow === 'left')
                                <figure class="avatar"><img src="{{ $company->profile_image }}"></figure>
                            @endif
                            
                            <div class="timestamp timestamp--{{ $chat->arrow }}">
                                <span>{{ $chat->message_register_hour }}</span><span>:</span><span>{{ $chat->message_register_min }}</span>
                            </div>
                    ...
                    @elseif($chat->type == '2')
                        ...
                        <div class="chat-box chat-box--{{ $chat->arrow }} {{ $chat->arrow === 'left' ? 'chat-box--avatar' : '' }}" tabindex="0">
                            <div class="chat chat--{{ $chat->arrow }} chat--photo" style="height: 390px; background-image: url({{ $chat->content }});" onclick="openImageModal('{{ $chat->content }}')">
                            </div>
                            @if($chat->arrow === 'left')
                                <figure class="avatar"><img src="{{ $company->profile_image }}"></figure>
                            @endif
                    ...
                    @else
                        ...
                        <div tabindex="0" class="chat-box chat-box--{{ $chat->arrow }} {{ $chat->arrow === 'left' ? 'chat-box--avatar' : '' }} {{ $company->company_type === 'A' || json_decode($chat->content, true)['type'] === 'order' ? 'chat-box--cs' : '' }} {{ json_decode($chat->content, true)['type'] === 'inquiry' ? 'chat-box--product' : '' }}">
                            @if($chat->arrow === 'left')
                                <figure class="avatar"><img src="{{ $company->profile_image }}"></figure>
                            @endif
                            @if (json_decode($chat->content, true)['type'] === 'welcome')
                                <div class="chat chat--cs chat--left">
                                    <div class="chat-box__head">올펀 가입을 축하드립니다.</div>
                                    <div class="cs-info__title">
                                        더 편리한 서비스 이용을 위해 가이드를 확인해보세요!
                                    </div>
                                    <a href="javascript:openModal('#guide-modal')" role="button" class="cs-info__button">올펀 이용 가이드</a>
                                    <a href="javascript:sendMessageHelpCenter({{ $room_idx }})" role="button" class="cs-info__button">올펀 고객센터</a>
                                </div>
                            @elseif (json_decode($chat->content, true)['type'] === 'cs')
                                <div class="chat chat--cs chat--left">
                                    <div class="chat-box__head">올펀 고객센터</div>
                                    <div class="cs-info__title">
                                        전화번호: {{ config('constants.ALLFURN.CALL_NUMBER') }}
                                        <br>운영시간: {{ config('constants.ALLFURN.OPERATION_TIME') }}
                                    </div>
                                    <a href="/help/faq" role="button" class="cs-info__button">자주 묻는 질문</a>
                                    <a href="/help/notice" role="button" class="cs-info__button">공지사항</a>
                                </div>
                            @elseif (json_decode($chat->content, true)['type'] === 'inquiry')
                                <div class="chat chat--product chat--{{ $chat->arrow }}">
                                    <div class="chat-box__head">
                                        @if($keyword)
                                            {!! str_replace($keyword, '<div class="selection">'.$keyword.'</div>', json_decode($chat->content, true)['msg']) !!}
                                        @else
                                            {{ json_decode($chat->content, true)['msg'] }}
                                        @endif
                                    </div>
                                    <div class="chat-box__inner" onclick="location.href='/product/detail/{{ json_decode($chat->content, true)['productIdx'] }}'">
                                        <div class="product-thumnail" style="background-image: url({{ json_decode($chat->content, true)['productThumbnailURL'] }});"></div>
                                        <div class="product-info">
                                            <div class="product-info__title">
                                                @if($keyword)
                                                    {!! str_replace($keyword, '<div class="selection">'.$keyword.'</div>', json_decode($chat->content, true)['productName']) !!}
                                                @else
                                                    {{ json_decode($chat->content, true)['productName'] }}
                                                @endif
                                            </div>
                                            <div class="product-info__desc">
                                                <span class="price">
                                                    @if($keyword)
                                                        {!! str_replace($keyword, '<div class="selection">'.$keyword.'</div>', json_decode($chat->content, true)['productPrice']) !!}
                                                    @else
                                                        @if (gettype(json_decode($chat->content, true)['productPrice']) === 'string')
                                                            {{ json_decode($chat->content, true)['productPrice'] }}
                                                        @else
                                                            {{ number_format(json_decode($chat->content, true)['productPrice']) }}
                                                        @endif
                                                    @endif
                                                </span>
                                                @if (gettype(json_decode($chat->content, true)['productPrice']) !== 'string')
                            ...
                            @elseif (in_array(json_decode($chat->content, true)['type'],['order','order_complete']))
                                <div class="chat chat--cs chat--{{ $chat->arrow }}">
                                    <div class="chat-box__head">주문번호: <span>
                                            @if($keyword)
                                                @if (json_decode($chat->content, true)['type'] === 'order_complete')
                                                    <div class="selection">{{ json_decode($chat->content, true)['orderNumber'] }}</div>
                                                @else
                                                    {!! str_replace($keyword, '<div class="selection">'.$keyword.'</div>', json_decode($chat->content, true)['order_group_code']) !!}
                                                @endif
                                            @else
                                                @if (json_decode($chat->content, true)['type'] === 'order_complete')
                                                    {{ json_decode($chat->content, true)['orderNumber'] }}
                                                @else
                                                    {{ json_decode($chat->content, true)['order_group_code'] }}
                                                @endif
                                            @endif
                                        </span>
                                    </div>
                                    ...
                                    <div class="cs-info__title">
                                        @if ($keyword)
                                            @if(json_decode($chat->content, true)['type'] !== 'order_complete')
                                                {!! str_replace($keyword, '<div class="selection">'.$keyword.'</div>', json_decode($chat->content, true)['text']) !!}
                                            @endif
                                        @else
                                            @if(json_decode($chat->content, true)['type'] === 'order_complete')
                                                주문이 완료되었습니다.<br/>감사합니다.
                                            @else
                                                {!! json_decode($chat->content, true)['text'] !!}
                                            @endif
                                        @endif
                                    </div>
                                    
                                    @if(json_decode($chat->content, true)['type'] === 'order_complete')
                                        <a href="/mypage/order/detail?type=P&orderGroupCode={{ json_decode($chat->content, true)['orderNumber'] }}" class="cs-info__button button--blank">주문 현황 보러가기</a>
                                    @else
                                        <a href="/mypage/order/detail?type=P&orderGroupCode={{ json_decode($chat->content, true)['order_group_code'] }}" class="cs-info__button button--blank">주문 현황 보러가기</a>
                                    @endif
                    ...
                    @endif
{{--                <div class="chat-box chat-box--left chat-box--avatar">--}}
{{--                    <div class="chat chat--left chat--disabled">--}}
{{--                        이미지가 만료되었습니다.--}}
{{--                    </div>--}}
{{--                    <figure class="avatar"><img src="/images/temp/dummy-logo.png"></figure>--}}
            ...
            @if($company->company_type !== 'A')
                <div class="section__bottom">
                    ...
                    <div class="product-quotation">
                        <input type="hidden" name="product_idx" id="product_idx" value="{{ $product->idx }}" />
                        <div class="row">{{ $product->name }}</div>
                        <div class="row cate">상품 문의</div>
                        <div class="section__bottom-button">
                            <a role="button" class="bottom-button" onclick="deleteRoomProduct(this)">
                    ...
                    @endif
                    <div class="section__bottom-button">
                        <input type="file" name="image" id="image" accept="image/jpg, image/png, image/jpeg" class="hidden"/>
                        <a role="button" class="bottom-button" onclick="selectImage()"><div class="ico__gallery-image"><span class="a11y">사진 불러오기</span></div></a>
                    </div>
                    <div class="textfield" id="previewWrap">
                        <input  class="message-input textfield__input" type="text" id="chat_message" name="chat_message" placeholder="메시지를 입력해주세요." />
                        <div class="photo-container hidden" id="selectedImagePreview">
                            <div class="photo" id="preview" style="">
                                <span class="a11y">첨부 이미지</span>
                                <button type="button" class="button ico__sdelete" onclick="deletePreviewImage()"><span class="a11y">삭제하기</span></button>
                            </div>
                        </div>
                    </div>
                    <button type="submit" {{ isset($product) ? 'disabled="true"' : '' }}disabled="true" id="submitBtn" class="message-submit" data-room-idx="{{ $room_idx }}" onclick="submitMessage(this)">전송</button>
                </div>
            @else
                <div class="section__bottom section__bottom--chatbot">
                    <div class="welcome">
                        <div class="section__bottom-button">
                            <a role="button" class="sticked-button" href="/help/faq">이용 안내</a>
                            <a role="button" class="sticked-button" href="/help/inquiry">1:1 문의</a>
                        </div>
                    </div>
                </div>
            @endif
