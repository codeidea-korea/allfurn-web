<div class="w-full">
    <div class="flex justify-between">
        <h3 class="text-xl font-bold">좋아요 업체</h3>
    </div>
    <div class="mt-4">
        ...
        @if(request()->input('regions') != null)
            ...
            $("#filter_location-modal .check-form").each(function() {
                if(selectedRegions.indexOf($(this).siblings('label').text()) != -1) {
                    $(this).prop('checked', true);
                    $(".location").append('<span>' + $(this).siblings('label').text() +'<button data-id = "'+ $(this).attr('id') +'" onclick="filterRemove(this)"><svg><use xlink:href="/img/icon-defs.svg#x"></use></svg></button></span>')
                    $(".sub_filter_result").css('display', 'flex');
                }
            });
        @endif
