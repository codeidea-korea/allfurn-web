@foreach ($list as $key => $item )
    <div class="prod_info">
        ...
        @if(isset($item->product_option) && $item->product_option != '[]')
            <?php $arr = json_decode($item->product_option); $required = false; ?>
            @foreach($arr as $item2)
            
            <div class="info_box">
                <div class="prod_name">{{$item2->optionName}}</div>
                <div class="dropdown_wrap noline">
                    <button class="dropdown_btn"><p>{{$item2->optionName}} 선택
                            @if($item2->required == 1)
                                (필수)
                            @else
                                (선택)
                    ...
                    <div class="dropdown_list">
                        @foreach($item2->optionValue as $sub)
                            <div class="dropdown_item" data-option_name="{{$sub->propertyName}}" data-price="{{$sub->price}}">
                                    {{$sub->propertyName}}
                ...
                <div class="noline">
                    ...
                    @foreach($item2->optionValue as $sub)
                        <div class="option_item" data-option_name="{{$sub->propertyName}}" data-price="{{$sub->price}}">
                            <div class="">
