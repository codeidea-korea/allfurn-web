...
class HomeService
{
    ...
    public function getHomeData()
    {
        ...
        $banner_top_query = Banner::select(
            ...
            , 'AF_banner_ad.web_link', 'ac.folder', 'ac.filename'
            , 'AF_banner_ad.banner_type', 'AF_banner_ad.bg_color', 'AF_banner_ad.font_color'
            , 'AF_banner_ad.subtext1', 'AF_banner_ad.subtext2'
        );
        if (getDeviceType() == "m.") {
            $banner_top_query->leftjoin('AF_attachment as ac', 'ac.idx', 'AF_banner_ad.appbig_attachment_idx');
        }else{
            $banner_top_query->leftjoin('AF_attachment as ac', 'ac.idx', 'AF_banner_ad.web_attachment_idx');
        }
        ...
        $data['plandiscount_ad'] = Banner::select('AF_banner_ad.*', 
            ...
            ->leftjoin('AF_attachment as at', function($query) {
                $query->on('at.idx', 'AF_banner_ad.web_attachment_idx');
            })
            ->leftjoin('AF_attachment as app', function($query) {
                $query->on('app.idx', 'AF_banner_ad.appbig_attachment_idx');
            })
            ->leftjoin('AF_attachment as app51', function($query) {
                $query->on('app51.idx', 'AF_banner_ad.app51_attachment_idx');
            })
            ->where('AF_banner_ad.state', 'G')
            ->where('AF_banner_ad.start_date', '<', DB::raw("now()"))
            ->where('AF_banner_ad.end_date', '>', DB::raw("now()"))
