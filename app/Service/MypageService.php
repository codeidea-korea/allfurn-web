...
class MypageService
{
    ...
    public function changeStatus(array $params): array
    {
        ...
        */
        ...
        $pushService -> sendPush(
            $title_request, $message_request, $orders[0] -> user_idx, $type = 5, env('APP_URL').'/mypage/purchase', env('APP_URL').'/mypage/purchase'
        );

        // 판매자
        $pushService -> sendPush(
            $title_response, $message_response, $product[0]->userIdx, $type = 5, env('APP_URL').'/mypage/deal', env('APP_URL').'/mypage/deal'
        );
    ...
    public function deleteProduct($idx): array
    {
        Product::where('company_idx', Auth::user()['company_idx'])
            ->where('company_type', Auth::user()['type'])
            ->where('idx', $idx)->update(['deleted_at' => date('Y-m-d H:i:s'), 'state' => 'C']);
        return [
            'result' => 'success',
            'message' => ''
        ];
